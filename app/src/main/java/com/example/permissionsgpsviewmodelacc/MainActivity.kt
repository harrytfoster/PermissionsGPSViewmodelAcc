...
class MainActivity : ComponentActivity(), LocationListener { // LocationListener handles location updates
    ...
    fun checkPermissions() {
        val requiredPermission = Manifest.permission.ACCESS_FINE_LOCATION // Choose permission to use
        if(checkSelfPermission(requiredPermission) == PackageManager.PERMISSION_GRANTED) { // Checks is permission is already granted
            ...
            val permissionLauncher = registerForActivityResult(ActivityResultContracts.RequestPermission()) { isGranted -> // Request Permission
                if(isGranted) {
                    startGPS()
                } else {
                    Toast.makeText(this, "GPS permission not granted", Toast.LENGTH_LONG).show() // Show this is permission is not granted
                }
            }
    ...
    fun startGPS() {
        val mgr = getSystemService(LOCATION_SERVICE) as LocationManager // System obtains default location manager
        mgr.requestLocationUpdates(LocationManager.GPS_PROVIDER, 0, 0f, this ) // Update app with new location information
    }
    ...
    fun DisplayCoords(){
        ...
        viewModel.latLonLiveData.observe(this){
            var Obslat = it.lat.toString()
            var Obslon = it.lon.toString()
        }
    }
}